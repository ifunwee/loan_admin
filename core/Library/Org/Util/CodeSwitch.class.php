...
class CodeSwitch
{
    ...
    static function DetectAndSwitch($filename, $out_charset)
    {
        ...
        if (strtoupper($out_charset) == $originEncoding) {
            ...
            //转码并保存文件
            $content = iconv(str_replace(" WITH BOM", "", $originEncoding),
                strtoupper($out_charset), $originContent);
    ...
    static function searchdir(
        $path, $mode = "FULL", $file_types = array(
        ".html",
        ".php"
    ), $maxdepth = -1, $d = 0
    ) {
            ...
            while (false !== ($file = readdir($handle))) {
                if ($file != '.' && $file != '..') {
                    ...
                    if (!is_dir($file)) {
                        ...
                        $result  = self::searchdir($file . '/', $mode, $file_types, $maxdepth,
                            $d + 1);
     ...
     * @return void
     */
    static function CodingSwitch(
        $app = "./", $charset = 'UTF-8', $mode = "FILES", $file_types = array(
        ".html",
        ".php"
    )
    ) {
