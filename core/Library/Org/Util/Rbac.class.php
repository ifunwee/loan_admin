...
class Rbac
{
    ...
    static function checkAccess()
    {
        ...
        if (C('USER_AUTH_ON')) {
            ...
            //检查当前模块是否需要认证
            if ((!empty($_module['no']) && !in_array(strtoupper(CONTROLLER_NAME),
                        $_module['no'])) || (!empty($_module['yes']) && in_array(strtoupper(CONTROLLER_NAME),
                        $_module['yes']))
            ) {
    ...
    static public function getAccessList($authId)
    {
        ...
        foreach ($apps as $key => $app) {
            ...
            foreach ($modules as $key => $module) {
                ...
                // 和公共模块的操作权限合并
                $action                                                += $publicAction;
                $access[strtoupper($appName)][strtoupper($moduleName)] = array_change_key_case($action,
                    CASE_UPPER);
            }
        }
