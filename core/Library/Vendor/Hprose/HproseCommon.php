...
class HproseTime
{
    ...
    {
        if ($this->utc) {
            return gmmktime($this->hour, $this->minute,
                    $this->second) + ($this->microsecond / 1000000);
        } else {
...
class HproseDateTime extends HproseDate
{
    ...
    {
        if ($this->utc) {
            return gmmktime($this->hour, $this->minute, $this->second, $this->month, $this->day,
                    $this->year) + ($this->microsecond / 1000000);
        } else {
    ...
    public function toString($fullformat = true)
    {
        if ($this->microsecond == 0) {
            ...
            $str    = sprintf($format, $this->year, $this->month, $this->day, $this->hour,
                $this->minute, $this->second);
        }
        if ($this->microsecond % 1000 == 0) {
            $format = ($fullformat ? '%04d-%02d-%02dT%02d:%02d:%02d.%03d' : '%04d%02d%02dT%02d%02d%02d.%03d');
            $str    = sprintf($format, $this->year, $this->month, $this->day, $this->hour,
                $this->minute, $this->second, (int)($this->microsecond / 1000));
        } else {
