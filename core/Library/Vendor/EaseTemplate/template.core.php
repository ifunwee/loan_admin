...
class ETCore
{
    ...
    function ETCoreStart(
        ...
        if (@ereg(':', $set['MemCache'])) {
            ...
            $this->RunType = (@substr(@sprintf('%o', @fileperms($this->CacheDir)),
                    -3) == 777 && is_dir($this->CacheDir)) ? 'Cache' : 'Replace';
        }
        ...
        $this->Compile = (is_array($this->Compile)) ? array_merge($this->Compile,
        ...
        $this->Analysis = (is_array($this->Analysis)) ? array_merge($this->Analysis,
            $AnalysisBasic) : $AnalysisBasic;
    }
    ...
    function ParseCode(
        ...
        if (eregi("run:", $ShowTPL)) {
            ...
            //Fix Run 1
            $ShowTPL = preg_replace('/(\{|<!--\s*)run:(\}|\s*-->)\s*(.+?)\s*(\{|<!--\s*)\/run(\}|\s*-->)/is',
                '(T_T)\\3;(T_T!)', $ShowTPL);
        }
        ...
        //Fix Run 2
        if ($run == 1) {
            $ShowTPL = preg_replace("/\(T_T\)(.+?)\(T_T!\)/ise", '$this->FixPHP(\'\\1\')',
                $ShowTPL);
        }

        //还原xml
        $ShowTPL = (strrpos($ShowTPL, '<ET>')) ? @preg_replace('/ET>(.+?)<\/ET/is', '?\\1?',
            $ShowTPL) : $ShowTPL;
    ...
    function lang(
        ...
        if (is_dir($this->LangDir)) {
            ...
            if ($this->LangData[$id] == '' && $this->Language == 'default') {
                ...
                //语言文件过大时采取建立新文件
                if (strlen($docs) > 400) {
                    $this->writer($this->LangDir . $this->Language . '.' . $id . '.php',
                        '<? $etl = "' . $str . '";?>');
    ...
    function inc_preg(
        ...
        return preg_replace('/<\!--\s*\#include\s*file\s*=(\"|\')([a-zA-Z0-9_\.\|]{1,100})(\"|\')\s*-->/eis',
            '$this->inc("\\2")',
            preg_replace('/(\{\s*|<!--\s*)inc\:([^\{\} ]{1,100})(\s*\}|\s*-->)/eis',
                '$this->inc("\\2")', $content));
    }
...
<!-- END -->";
                ...
                //cache date
                memcache_set($this->Emc, $Files . '_date',
                    time()) OR die("Failed to save data at the server.");
                memcache_set($this->Emc, $Files,
                    $contents) OR die("Failed to save data at the server");
    ...
    function CompilePHP(
        ...
        if ($content) {
            ...
            $this->writer($this->FileName($wfile, $this->TplID),
                $this->Hacker . '$EaseTemplate3_Cache = \'' . $content . '\';');
        }
    }
                    ...
                    if (trim($CacheData) && $update) {
                        ...
                        if (is_array($IncFile[1])) {
                            ...
                            if ($update == 1) {
                                ...
                                //cache date
                                @memcache_set($this->Emc, $CacheFile . '_date',
                                    time()) OR die("Failed to save data at the server.");
                                @memcache_set($this->Emc, $CacheFile,
                                    $CacheData) OR die("Failed to save data at the server.");
                            }
                        }
                        //Close
                        ...
                        if ($Filename) {
                            ...
                            @memcache_set($this->Emc, $CacheFile . '_date',
                                time()) OR die("Failed to save data at the server.");
                            @memcache_set($this->Emc, $CacheFile,
                                $CacheData) OR die("Failed to save data at the server2");
                            //Close
                            memcache_close($this->Emc);
                ...
                if ($Filename == 'include_page') {
                    ...
                    if (@is_file($CacheFile)) {
                        ...
                        if ($EaseTemplate3_Cache != '') {
                            unset($ks, $vs);
                            preg_match_all('/<\!-- ET\_inc\_cache\[(.+?)\] -->/',
                                $EaseTemplate3_Cache, $IncFile);
    ...
    function htm_w(
        ...
        if ($w_dir && $w_filename && $w_content) {
            ...
            foreach ((array)$this->FileDir AS $fdk => $fdv) {
                $w_content = str_replace($fdv, $w_dir_len . str_replace('../', '', $fdv),
                    $w_content);
            }
    ...
    function Dirs(
        ...
        if (is_array($adds_ary)) {
            $this->ImgDir = (is_array($this->ImgDir)) ? @array_merge($adds_ary,
                $this->ImgDir) : $adds_ary;
        }
    }
    ...
    function reader(
        ...
        return (in_array('file_get_contents',
            $get_fun['internal'])) ? @file_get_contents($filename) : @implode("", @file($filename));
    }
