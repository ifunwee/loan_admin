...
class ShallowParser
{
    ...
    {
        $this->_initials = '/^(' . implode('|',
                array_map(array($this, 'quote'), array_keys($this->_pairs))) . ')/';
    }
    ...
    private function _resetResult($result)
    {
        ...
        $result->terminator = $result->state ? '/^(.*?' . preg_quote($this->_pairs[$result->state],
                '/') . ')/s' : null;
    }
    ...
    {
        if (($result->state == '"' || $result->state == "'") && preg_match('/^[^' . $result->state . ']*?\\\\./s',
                $result->buffer, $match)
        ) {
    ...
    {
        return preg_match('/^([^=]*?=\s*)?function\s*\([^\)]*\)\s*(use\s*\([^\)]*\)\s*)?\s*\{.*\}\s*;?$/is',
            trim($input));
    }
    ...
    private function _isReturnable($input)
    {
        ...
        if (substr($input, -1) == ';' && substr($input, 0, 1) != '{') {
            return $this->_isLambda($input) || !preg_match('/^(' . 'echo|print|exit|die|goto|global|include|include_once|require|require_once|list|' . 'return|do|for|foreach|while|if|function|namespace|class|interface|abstract|switch|' . 'declare|throw|try|unset' . ')\b/i',
                    $input);
        } else {
            return false;
        }
