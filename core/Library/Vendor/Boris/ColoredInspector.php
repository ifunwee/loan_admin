...
class ColoredInspector implements Inspector
{
    ...
    {
        return $this->_dumpStructure(sprintf('object(%s)', get_class($value)),
    ...
    public function _buildAst($type, $value, $seen = array())
    {
        ...
        return array(
            ...
            'children' => empty($vars) ? array() : array_combine(array_map(array($this, '_dump'),
                array_keys($vars)), array_map(function ($v) use ($self, $nextSeen) {
                    if (is_object($v)) {
                        return $self->_buildAst(sprintf('object(%s)', get_class($v)), $v,
                            $nextSeen);
                    } elseif (is_array($v)) {
        ...
        return implode("\n", array(
                ...
                implode(",\n", array_map(function ($k) use ($self, $children, $indent) {
                        if (is_array($children[$k])) {
                            return sprintf('%s%s => %s', str_repeat(' ', ($indent + 1) * 2), $k,
