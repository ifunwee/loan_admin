...
class _PHPRPC_Client
{
    ...
    function useService($serverURL, $username = null, $password = null)
    {
        ...
        if (!isset($urlparts['path'])) {
            ...
            if (($urlparts['path']{0} != '/') && ($_SERVER["PHP_SELF"]{0} == '/')) {
                $urlparts['path'] = substr($_SERVER["PHP_SELF"], 0,
                        strrpos($_SERVER["PHP_SELF"], '/') + 1) . $urlparts['path'];
            }
        }
    ...
    function _connect()
    {
        if (is_null($this->_proxy)) {
            ...
            $this->_socket = @pfsockopen($host, $this->_server['port'], $errno, $errstr,
                $this->_timeout);
        } else {
            $host          = (($this->_server['scheme'] == "https") ? "ssl://" : "") . $this->_proxy['host'];
            $this->_socket = @pfsockopen($host, $this->_proxy['port'], $errno, $errstr,
                $this->_timeout);
        }
    ...
    function _parseHeader($header)
    {
        ...
        if (preg_match('/PHPRPC Server\/([^,]*)(,|$)/i', implode(',', $header['x-powered-by']),
            $match)) {
            $this->_server['version'] = (float)$match[1];
        } else {
            return new PHPRPC_Error(E_ERROR, "Illegal PHPRPC server.");
        }
        if (preg_match('/text\/plain\; charset\=([^,;]*)([,;]|$)/i', $header['content-type'][0],
            $match)) {
    ...
    function _key_exchange()
    {
        ...
        if (array_key_exists('phprpc_encrypt', $result)) {
            ...
            $encrypt    = bigint_num2dec(bigint_powmod(bigint_dec2num($encrypt['g']), $x,
                bigint_dec2num($encrypt['p'])));
