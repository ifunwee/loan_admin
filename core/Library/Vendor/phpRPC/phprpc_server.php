...
class PHPRPC_Server
{
    ...
    {
        if ($this->encrypt >= 3) {
            $this->buffer .= "phprpc_output=\"" . $this->encodeString(xxtea_encrypt($output,
                    $this->key)) . "\";\r\n";
        } else {
    ...
    function fatalErrorHandler($buffer)
    {
        if (preg_match('/<b>(.*?) error<\/b>:(.*?)<br/', $buffer, $match)) {
            ...
            $buffer = "phprpc_errno=\"{$errno}\";\r\n" . "phprpc_errstr=\"" . $this->encodeString(trim($errstr),
                    false) . "\";\r\n" . "phprpc_output=\"\";\r\n" . $this->callback;
    ...
    function getArguments()
    {
        if (isset($_REQUEST['phprpc_args'])) {
            $arguments = unserialize($this->decryptString(base64_decode($this->getRequest('phprpc_args')),
                1));
            ksort($arguments);
        } else {
            $arguments = array();
    ...
    function callFunction()
    {
        ...
        if (array_key_exists($function, $this->functions)) {
            ...
            $result       = $this->encodeString($this->encryptString(serialize_fix($this->call($function,
                $arguments)), 2));
            $output       = ob_get_clean();
            $this->buffer .= "phprpc_result=\"$result\";\r\n";
            if ($this->ref) {
                $arguments    = $this->encodeString($this->encryptString(serialize_fix($arguments),
                    1));
    ...
    function keyExchange()
    {
        ...
        if ($this->encrypt === true) {
            ...
            $encrypt['y']      = bigint_num2dec(bigint_powmod(bigint_dec2num($encrypt['g']), $x,
