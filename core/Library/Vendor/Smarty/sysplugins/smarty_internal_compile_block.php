...
class Smarty_Internal_Compile_Block extends Smarty_Internal_CompileBase
{
    ...
    public static function saveBlockData($block_content, $block_tag, $template, $filepath)
    {
        ...
        if (0 == preg_match("!({$_ldl}block\s+)(name=)?(\w+|'.*'|\".*\")(\s*?)?((append|prepend|nocache)?(\s*)?(hide)?)?(\s*{$_rdl})!",
                $block_tag, $_match)
        ) {
            ...
            if ($_match[8] != 'hide' || isset($template->block_data[$_name])) {        // replace {$smarty.block.child}
                if (strpos($block_content,
                        $template->smarty->left_delimiter . '$smarty.block.child' . $template->smarty->right_delimiter) !== false
                ) {
                ...
                if (isset($template->block_data[$_name])) {
                    if (strpos($template->block_data[$_name]['source'],
                        ...
                        $template->block_data[$_name]['source'] = str_replace('%%%%SMARTY_PARENT%%%%',
                            $block_content, $template->block_data[$_name]['source']);
    ...
    public static function compileChildBlock($compiler, $_name = null)
    {
        ...
        $_tpl                             = new Smarty_Internal_template ('string:' . $compiler->template->block_data[$_name]['source'],
            ...
            $compiler->template->compile_id = null, $compiler->template->caching,
        ...
        $compiler->template->properties['file_dependency'] = array_merge($compiler->template->properties['file_dependency'],
            $_tpl->properties['file_dependency']);
        $compiler->template->properties['function']        = array_merge($compiler->template->properties['function'],
            $_tpl->properties['function']);
        $compiler->merged_templates                        = array_merge($compiler->merged_templates,
