        ...
        */
        ...
        if (($compiler->smarty->merge_compiled_includes || $_attr['inline'] === true) && !$compiler->template->source->recompiled && !($compiler->template->caching && ($compiler->tag_nocache || $compiler->nocache)) && $_caching != Smarty::CACHING_LIFETIME_CURRENT) {
            ...
            if (!$compiler->has_variable_string && (substr_count($include_file,
                        ...
                        "'") == 2) and substr_count($include_file,
                    '(') == 0 and substr_count($include_file, '$_smarty_tpl->') == 0
            ) {
                $tpl_name = null;
                ...
                if (!isset($compiler->smarty->merged_templates_func[$tpl_name]) || $compiler->inheritance) {
                    $tpl = new $compiler->smarty->template_class ($tpl_name, $compiler->smarty,
                        $compiler->template, $compiler->template->cache_id,
                    ...
                    if (!($tpl->source->uncompiled) && $tpl->source->exists) {
                        ...
                        if ($tpl->has_nocache_code) {
                            ...
                            $compiled_code                        = preg_replace("/{$tpl->properties['nocache_hash']}/",
                                $compiler->template->properties['nocache_hash'], $compiled_code);
