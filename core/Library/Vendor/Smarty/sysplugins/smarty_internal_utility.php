...
class Smarty_Internal_Utility
{
    ...
    public static function compileAllTemplates($extention, $force_compile, $time_limit, $max_errors, Smarty $smarty)
    {
        ...
        foreach ($smarty->getTemplateDir() as $_dir) {
            ...
            foreach ($_compile as $_fileinfo) {
                if (substr($_fileinfo->getBasename(), 0, 1) == '.' || strpos($_fileinfo,
                        '.svn') !== false
                ) {
                    continue;
                }
    ...
    public static function clearCompiledTemplate($resource_name, $compile_id, $exp_time, Smarty $smarty)
    {
        ...
        if (isset($resource_name)) {
            ...
            if ($tpl->source->exists) {
                $_resource_part_1        = basename(str_replace('^', '/',
        ...
        foreach ($_compile as $_file) {
            ...
            if ($_file->isDir()) {
                ...
                $unlink = false;
                if ((!isset($_compile_id) || (isset($_filepath[$_compile_id_part_length]) && !strncmp($_filepath,
                                $_compile_id_part,
                                $_compile_id_part_length))) && (!isset($resource_name) || (isset($_filepath[$_resource_part_1_length]) && substr_compare($_filepath,
                                $_resource_part_1, -$_resource_part_1_length,
                                $_resource_part_1_length) == 0) || (isset($_filepath[$_resource_part_2_length]) && substr_compare($_filepath,
                                $_resource_part_2, -$_resource_part_2_length,
    ...
    public static function testInstall(Smarty $smarty, &$errors = null)
    {
        ...
        foreach ($smarty->getTemplateDir() as $template_dir) {
            ...
            if (!$template_dir) {
                if ($smarty->use_include_path && !preg_match('/^([\/\\\\]|[a-zA-Z]:[\/\\\\])/',
                        $_template_dir)
                ) {
        ...
        if (is_dir($source)) {
            ...
            if ($expected) {
                $status  = false;
                $message = "FAILED: files missing from libs/plugins: " . join(', ',
                        array_keys($expected));
                if ($errors === null) {
                    echo $message . ".\n";
                } else {
