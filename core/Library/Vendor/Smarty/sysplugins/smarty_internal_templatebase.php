...
abstract class Smarty_Internal_TemplateBase extends Smarty_Internal_Data
{
    ...
    public function fetch($template = null, $cache_id = null, $compile_id = null, $parent = null, $display = false, $merge_tpl_vars = true, $no_output_filter = false)
    {
        ...
        $_template = ($template instanceof $this->template_class) ? $template : $this->smarty->createTemplate($template,
            $cache_id, $compile_id, $parent, false);
        ...
        if (!$this->smarty->debugging && $this->smarty->debugging_ctrl == 'URL') {
            ...
            if (false !== strpos($_query_string, $this->smarty->smarty_debug_id)) {
                ...
                } elseif (false !== strpos($_query_string,
                        $this->smarty->smarty_debug_id . '=off')
                ) {
        ...
        if (!($_template->caching == Smarty::CACHING_LIFETIME_CURRENT || $_template->caching == Smarty::CACHING_LIFETIME_SAVED) || !$_template->cached->valid) {
            ...
            if ($_template->parent instanceof Smarty_Internal_Template) {
                $_template->parent->properties['file_dependency'] = array_merge($_template->parent->properties['file_dependency'],
            ...
            if (!$_template->source->recompiled && ($_template->caching == Smarty::CACHING_LIFETIME_SAVED || $_template->caching == Smarty::CACHING_LIFETIME_CURRENT)) {
                ...
                $cache_split = preg_split("!/\*%%SmartyNocache:{$_template->properties['nocache_hash']}%%\*\/(.+?)/\*/%%SmartyNocache:{$_template->properties['nocache_hash']}%%\*/!s",
                    $_output);
                ...
                foreach ($cache_split as $curr_idx => $curr_split) {
                    ...
                    if (isset($cache_parts[0][$curr_idx])) {
                        ...
                        $output .= preg_replace("!/\*/?%%SmartyNocache:{$_template->properties['nocache_hash']}%%\*/!",
                ...
                if (!$no_output_filter && (isset($this->smarty->autoload_filters['output']) || isset($this->smarty->registered_filters['output']))) {
                    $output = Smarty_Internal_Filter_Handler::runFilter('output', $output,
                        $_template);
                }
                ...
                if (!empty($_template->properties['nocache_hash']) && !empty($_template->parent->properties['nocache_hash'])) {
                    ...
                    $_output                             = preg_replace("/{$_template->properties['nocache_hash']}/",
                        $_template->parent->properties['nocache_hash'], $_output);
                        ...
                        case 'cli':
                            ...
                            !empty($_SERVER['SMARTY_PHPUNIT_DISABLE_HEADERS'])/* phpunit$ */
                                ...
                                $_SERVER['SMARTY_PHPUNIT_HEADERS'][] = 'Last-Modified: ' . gmdate('D, d M Y H:i:s',
                                        $_template->cached->timestamp) . ' GMT';
                            }
                            break;
    ...
    public function isCached($template = null, $cache_id = null, $compile_id = null, $parent = null)
    {
        ...
        if (!($template instanceof $this->template_class)) {
            ...
            $template = $this->smarty->createTemplate($template, $cache_id, $compile_id, $parent,
                false);
        }
    ...
    public function __call($name, $args)
    {
        ...
        if (isset($_prefixes[$first3]) && isset($name[3]) && $name[3] !== '_') {
            if (isset($_resolved_property_name[$name])) {
                ...
                $property_name                  = preg_replace_callback('/([A-Z])/',
                    array($this, 'replaceCamelcase'), $property_name);
