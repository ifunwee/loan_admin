...
function smarty_function_html_select_date($params, $template)
{
        ...
        switch ($_key) {
            ...
            case 'month_names':
                if (is_array($_value) && count($_value) == 12) {
                    ...
                    trigger_error("html_select_date: month_names must be an array of 12 strings",
                        E_USER_NOTICE);
                }
                break;
            ...
            default:
                if (!is_array($_value)) {
                    ...
                    trigger_error("html_select_date: extra attribute '$_key' cannot be an array",
                        E_USER_NOTICE);
                }
                break;
        }
    ...
    if ($display_months) {
        ...
        for ($i = 1; $i <= 12; $i++) {
            $_val         = sprintf('%02d', $i);
            $_text        = isset($month_names) ? smarty_function_escape_special_chars($month_names[$i]) : ($month_format == "%m" ? $_val : strftime($month_format,
                $_month_timestamps[$i]));
            $_value       = $month_value_format == "%m" ? $_val : strftime($month_value_format,
                $_month_timestamps[$i]);
