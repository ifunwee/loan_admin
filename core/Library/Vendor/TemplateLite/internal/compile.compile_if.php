...
function compile_compile_if($arguments, $elseif, $while, &$object)
{
    ...
    preg_match_all('/(?>(' . $object->_var_regexp . '|\/?' . $object->_svar_regexp . '|\/?' . $object->_func_regexp . ')(?:' . $object->_mod_regexp . '*)?|\-?0[xX][0-9a-fA-F]+|\-?\d+(?:\.\d+)?|\.\d+|!==|===|==|!=|<>|<<|>>|<=|>=|\&\&|\|\||\(|\)|,|\!|\^|=|\&|\~|<|>|\%|\+|\-|\/|\*|\@|\b\w+\b|\S+)/x',
        $arguments, $_match);
    $_args = $_match[0];
    ...
    if (isset($_args_count['(']) && $_args_count['('] != $_args_count[')']) {
        $object->trigger_error("unbalanced parenthesis in if statement", E_USER_ERROR, __FILE__,
            __LINE__);
    }
        ...
        switch (strtolower($_arg)) {
            ...
            case 'is':
                if ($_args[$i - 1] == ')') {
                    ...
                    $is_arg        = implode(' ',
                        array_slice($_args, $is_arg_start, $i - $is_arg_start));
                ...
                break;
            default:
                preg_match('/(?:(' . $object->_var_regexp . '|' . $object->_svar_regexp . '|' . $object->_func_regexp . ')(' . $object->_mod_regexp . '*)(?:\s*[,\.]\s*)?)(?:\s+(.*))?/xs',
                    $_arg, $_match);
                if (isset($_match[0]{0}) && ($_match[0]{0} == '$' || ($_match[0]{0} == '#' && $_match[0]{strlen($_match[0]) - 1} == '#') || $_match[0]{0} == "'" || $_match[0]{0} == '"' || $_match[0]{0} == '%')) {
                    ...
                    $object->trigger_error("unidentified token '$_arg'", E_USER_ERROR, __FILE__,
                        __LINE__);
                }
                break;
        }
