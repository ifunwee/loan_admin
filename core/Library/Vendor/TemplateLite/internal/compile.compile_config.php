...
function compile_compile_config($variable, &$object)
{
    ...
    preg_match_all('!(?:^\w+)|(?:' . $object->_var_bracket_regexp . ')|\.\$?\w+|\S+!', $variable,
        $_match);
    $variable = $_match[0];
            ...
            if ($var{0} == '.') {
                ...
                if (substr($var, 0, 2) == '->') {
                    if (substr($var, 2, 2) == '__') {
                        $object->trigger_error('call to internal object members is not allowed',
                    ...
                    $object->trigger_error('#' . $var_name . implode('',
