...
class Template_Lite_Compiler extends Template_Lite
{
    ...
    function _compile_file($file_contents)
    {
        ...
        preg_match_all("!{$ldq}\s*literal\s*{$rdq}(.*?){$ldq}\s*/literal\s*{$rdq}!s",
            $file_contents, $_match);
        $this->_literal = $_match[1];
        $file_contents  = preg_replace("!{$ldq}\s*literal\s*{$rdq}(.*?){$ldq}\s*/literal\s*{$rdq}!s",
            stripslashes($ldq . "literal" . $rdq), $file_contents);

        // remove php blocks
        preg_match_all("!{$ldq}\s*php\s*{$rdq}(.*?){$ldq}\s*/php\s*{$rdq}!s", $file_contents,
            $_match);
        $this->_php_blocks = $_match[1];
        $file_contents     = preg_replace("!{$ldq}\s*php\s*{$rdq}(.*?){$ldq}\s*/php\s*{$rdq}!s",
            stripslashes($ldq . "php" . $rdq), $file_contents);
    ...
    function _compile_tag($tag)
    {
        ...
        preg_match_all('/(?:(' . $this->_var_regexp . '|' . $this->_svar_regexp . '|\/?' . $this->_func_regexp . ')(' . $this->_mod_regexp . '*)(?:\s*[,\.]\s*)?)(?:\s+(.*))?/xs',
            $tag, $_match);
    ...
    function _parse_function($function, $modifiers, $arguments)
    {
        switch ($function) {
            ...
            case 'insert':
                ...
                if (!isset($_args['name'])) {
                    $this->trigger_error("missing 'name' attribute in 'insert'", E_USER_ERROR,
                        __FILE__, __LINE__);
                }
                ...
                return '<?php echo $this->_run_insert(array(' . implode(', ',
                        (array)$arg_list) . ')); ?>';
                break;
            case 'ldelim':
            ...
            case 'foreach':
                ...
                $_args = $this->_parse_arguments($arguments);
                if (!isset($_args['from'])) {
                    $this->trigger_error("missing 'from' attribute in 'foreach'", E_USER_ERROR,
                        __FILE__, __LINE__);
                }
                if (!isset($_args['value']) && !isset($_args['item'])) {
                    $this->trigger_error("missing 'value' attribute in 'foreach'", E_USER_ERROR,
                        __FILE__, __LINE__);
                }
            ...
            case 'for':
                ...
                if (!isset($_args['start'])) {
                    $this->trigger_error("missing 'start' attribute in 'for'", E_USER_ERROR,
                        __FILE__, __LINE__);
                }
                if (!isset($_args['stop'])) {
                    $this->trigger_error("missing 'stop' attribute in 'for'", E_USER_ERROR,
                        __FILE__, __LINE__);
                }
                if (!isset($_args['step'])) {
                    $_args['step'] = 1;
            ...
            case 'assign':
                $_args = $this->_parse_arguments($arguments);
                if (!isset($_args['var'])) {
                    $this->trigger_error("missing 'var' attribute in 'assign'", E_USER_ERROR,
                        __FILE__, __LINE__);
                }
                if (!isset($_args['value'])) {
                    $this->trigger_error("missing 'value' attribute in 'assign'", E_USER_ERROR,
                        __FILE__, __LINE__);
                }
            ...
            case 'switch':
                $_args = $this->_parse_arguments($arguments);
                if (!isset($_args['from'])) {
                    $this->trigger_error("missing 'from' attribute in 'switch'", E_USER_ERROR,
                        __FILE__, __LINE__);
                }
                array_push($this->_switch_stack,
                    array("matched" => false, "var" => $this->_dequote($_args['from'])));
            ...
            case 'case':
                if (count($this->_switch_stack) > 0) {
                    ...
                    $this->trigger_error("unexpected 'case', 'case' can only be in a 'switch'",
            ...
            case 'config_load':
                $_args = $this->_parse_arguments($arguments);
                if (empty($_args['file'])) {
                    $this->trigger_error("missing 'file' attribute in 'config_load' tag",
                        E_USER_ERROR, __FILE__, __LINE__);
                }
            ...
            default:
                ...
                if ($this->_compile_compiler_function($function, $arguments, $_result)) {
                    ...
                    if ($this->_compile_custom_block($function, $modifiers, $arguments, $_result)) {
                        return $_result;
                    } elseif ($this->_compile_custom_function($function, $modifiers, $arguments,
                        $_result)
                    ) {
                        return $_result;
                    } else {
                        $this->trigger_error($function . " function does not exist", E_USER_ERROR,
                            __FILE__, __LINE__);
                    }
                }
                break;
    ...
    function _parse_arguments($arguments)
    {
        ...
        $_result    = array();
        $_variables = array();
        preg_match_all('/(?:' . $this->_qstr_regexp . ' | (?>[^"\'=\s]+))+|[=]/x', $arguments,
            $_match);
        /*
           Parse state:
        ...
        */
            ...
            switch ($state) {
                case 0:
                    ...
                    if (is_string($value)) {
                        ...
                        $this->trigger_error("invalid attribute name: '$token'", E_USER_ERROR,
                            __FILE__, __LINE__);
                    }
                    break;
                case 1:
                    if ($value == '=') {
                        ...
                        $this->trigger_error("expecting '=' after '$last_value'", E_USER_ERROR,
                            __FILE__, __LINE__);
                    }
                    break;
                case 2:
                    if ($value != '=') {
                        ...
                        if (!preg_match_all('/(?:(' . $this->_var_regexp . '|' . $this->_svar_regexp . ')(' . $this->_mod_regexp . '*))(?:\s+(.*))?/xs',
                            $value, $_variables)
                        ) {
                            $_result[$a_name] = $value;
                        } else {
                            $_result[$a_name] = $this->_parse_variables($_variables[1],
                                $_variables[2]);
                        }
                        $state = 0;
                    } else {
                        $this->trigger_error("'=' cannot be an attribute value", E_USER_ERROR,
                            __FILE__, __LINE__);
                    }
                    break;
            }
        ...
        if ($state != 0) {
            if ($state == 1) {
                $this->trigger_error("expecting '=' after attribute name '$last_value'",
                    E_USER_ERROR, __FILE__, __LINE__);
            } else {
    ...
    function _parse_variables($variables, $modifiers)
    {
        ...
        foreach ($variables as $key => $value) {
            ...
            if (!empty($this->default_modifiers) && !preg_match('!(^|\|)templatelite:nodefaults($|\|)!',
                    $modifiers[$key])
            ) {
            ...
            if (empty($modifiers[$key])) {
                ...
                $_result .= $this->_parse_modifier($this->_parse_variable($tag_variable),
                        $modifiers[$key]) . '.';
            }
        }
    ...
    function _parse_variable($variable)
    {
        ...
        if ($variable{0} == "\$") {
            ...
            $_result = "";
            preg_match_all('/(?:[^\\\]' . $this->_dvar_regexp . ')/', substr($variable, 1, -1),
                $_expand);  // old match
            ...
            foreach ($_expand as $value) {
                $value   = trim($value);
                $_result = str_replace($value, '" . ' . $this->_parse_variable($value) . ' . "',
                    $_result);
            }
    ...
    function _compile_variable($variable)
    {
        ...
        preg_match_all('!(?:^\w+)|(?:' . $this->_var_bracket_regexp . ')|\.\$?\w+|\S+!', $variable,
            $_match);
        $variable = $_match[0];
        ...
        if ($var_name == $this->reserved_template_varname) {
            if ($variable[0]{0} == '[' || $variable[0]{0} == '.') {
                ...
                $this->trigger_error('$' . $var_name . implode('',
                        $variable) . ' is an invalid $templatelite reference', E_USER_ERROR,
                    __FILE__, __LINE__);
            }
        } else {
                ...
                if ($var{0} == '.') {
                    ...
                    if (substr($var, 0, 2) == '->') {
                        if (substr($var, 2, 2) == '__') {
                            $this->trigger_error('call to internal object members is not allowed',
                                E_USER_ERROR, __FILE__, __LINE__);
                        } else {
                            if (substr($var, 2, 1) == '$') {
                                $_output .= '->{(($var=$this->_TPL[\'' . substr($var,
                                        3) . '\']) && substr($var,0,2)!=\'__\') ? $_var : $this->trigger_error("cannot access property \\"$var\\"")}';
                            }
                        }
                    } else {
                        $this->trigger_error('$' . $var_name . implode('',
                                $variable) . ' is an invalid reference', E_USER_ERROR, __FILE__,
                            __LINE__);
                    }
                }
            }
    ...
    function _parse_modifier($variable, $modifiers)
    {
        ...
        preg_match_all('!\|(@?\w+)((?>:(?:' . $this->_qstr_regexp . '|[^|]+))*)!', '|' . $modifiers,
            $_match);
    ...
    function _plugin_exists($function, $type)
    {
        ...
        if (isset($this->_plugins[$type][$function]) && is_array($this->_plugins[$type][$function]) && is_object($this->_plugins[$type][$function][0]) && method_exists($this->_plugins[$type][$function][0],
                $this->_plugins[$type][$function][1])
        ) {
