...
class [MODEL]Model extends Model {
    ...
    static public function buildAppDir($module)
    {
        ...
        if (is_writeable(APP_PATH)) {
            ...
            if (!is_file(CONF_PATH . 'config' . CONF_EXT)) {
                file_put_contents(CONF_PATH . 'config' . CONF_EXT,
                    '.php' == CONF_EXT ? "<?php\nreturn array(\n\t//'配置项'=>'配置值'\n);" : '');
            }
            // 写入模块配置文件
            if (!is_file(APP_PATH . $module . '/Conf/config' . CONF_EXT)) {
                file_put_contents(APP_PATH . $module . '/Conf/config' . CONF_EXT,
                    '.php' == CONF_EXT ? "<?php\nreturn array(\n\t//'配置项'=>'配置值'\n);" : '');
            }
            // 生成模块的测试控制器
    ...
    static public function buildController($module, $controller = 'Index')
    {
        ...
        if (!is_file($file)) {
            $content = str_replace(array('[MODULE]', '[CONTROLLER]'), array($module, $controller),
                self::$controller);
    ...
    static public function buildModel($module, $model)
    {
        ...
        if (!is_file($file)) {
            $content = str_replace(array('[MODULE]', '[MODEL]'), array($module, $model),
                self::$model);
            if (!C('APP_USE_NAMESPACE')) {
                $content = preg_replace('/namespace\s(.*?);/', '', $content, 1);
            }
