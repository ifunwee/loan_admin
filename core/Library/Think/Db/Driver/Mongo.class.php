...
class Mongo extends Driver
{
    ...
    public function getMongoNextId($pk)
    {
        ...
        try {
            $this->debug(true);
            $result = $this->_collection->find(array(),
                array($pk => 1))->sort(array($pk => -1))->limit(1);
            $this->debug(false);
        } catch (\MongoCursorException $e) {
            E($e->getMessage());
        }
    ...
    public function parseWhere($where)
    {
        ...
        if (isset($where['_logic'])) {
            ...
            $_logic          = in_array($where['_logic'],
                array('or', 'xor', 'nor', 'and')) ? '$' . $where['_logic'] : $_logic;
            unset($where['_logic']);
        }
    ...
    protected function parseWhereItem($key, $val)
    {
        ...
        if (is_array($val)) {
            if (is_string($val[0])) {
                ...
                if (in_array($con,
                    array('neq', 'ne', 'gt', 'egt', 'gte', 'lt', 'lte', 'elt'))) { // 比较运算
