...
class Shmop extends Cache
{
    ...
    private function _ftok($project)
    {
        ...
        if (strtoupper(PHP_OS) == 'WINNT') {
            ...
            return sprintf("%u",
                (($s['ino'] & 0xffff) | (($s['dev'] & 0xff) << 16) | (($project & 0xff) << 24)));
        } else {
            $filename = __FILE__ . (string)$project;
            for ($key = array(); sizeof($key) < strlen($filename); $key[] = ord(substr($filename,
                sizeof($key), 1))) {
                ;
            }
    ...
    private function _lock()
    {
        if (function_exists('sem_get')) {
            ...
            $fp = fopen($this->options['temp'] . $this->options['prefix'] . md5($this->handler),
                'w');
            flock($fp, LOCK_EX);
        }
