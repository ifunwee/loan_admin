...
class Model
{
    ...
    public function selectAdd($fields = '', $table = '', $options = array())
    {
        ...
        $options = $this->_parseOptions($options);
        // 写入数据到数据库
        if (false === $result = $this->db->selectInsert($fields ?: $options['field'],
                $table ?: $this->getTableName(), $options)
        ) {
            // 数据库插入操作失败
    ...
    protected function _parseType(&$data, $key)
    {
        if (!isset($this->options['bind'][':' . $key]) && isset($this->fields['_type'][$key])) {
            ...
            if (false !== strpos($fieldType, 'enum')) {
                // 支持ENUM类型优先检测
            } elseif (false === strpos($fieldType, 'bigint') && false !== strpos($fieldType,
                    'int')
            ) {
                $data[$key] = intval($data[$key]);
            } elseif (false !== strpos($fieldType, 'float') || false !== strpos($fieldType,
                    'double')
            ) {
            ...
            if (!empty($resultSet)) {
                ...
                foreach ($resultSet as $result) {
                    ...
                    if (2 == $count) {
                        ...
                        $cols[$name] = is_string($sepa) ? implode($sepa,
                            array_slice($result, 1)) : $result;
                    }
                }
                if (isset($cache)) {
                    S($key, $cols, $cache);
                    ...
                    switch (trim($auto[3])) {
                        ...
                        case 'callback': // 使用回调方法
                            ...
                            if ('function' == $auto[3]) {
                                ...
                                $data[$auto[0]] = call_user_func_array(array(&$this, $auto[1]),
                                    $args);
                            }
                            break;
    ...
    protected function parseSql($sql, $parse)
    {
        ...
        if (true === $parse) {
            ...
            $sql    = strtr($sql,
                array('__TABLE__' => $this->getTableName(), '__PREFIX__' => $this->tablePrefix));
    ...
    public function table($table)
    {
        ...
        if (is_array($table)) {
            ...
            $table                  = preg_replace_callback("/__([A-Z0-9_-]+)__/sU",
                function ($match) use ($prefix) {
                    return $prefix . strtolower($match[1]);
                }, $table);
            $this->options['table'] = $table;
    ...
    public function using($using)
    {
        ...
        if (is_array($using)) {
            ...
            //将__TABLE_NAME__替换成带前缀的表名
            $using                  = preg_replace_callback("/__([A-Z0-9_-]+)__/sU",
                function ($match) use ($prefix) {
                    return $prefix . strtolower($match[1]);
                }, $using);
            $this->options['using'] = $using;
    ...
    public function join($join, $type = 'INNER')
    {
        ...
        if (is_array($join)) {
            foreach ($join as $key => &$_join) {
                $_join = preg_replace_callback("/__([A-Z0-9_-]+)__/sU",
                    function ($match) use ($prefix) {
                        return $prefix . strtolower($match[1]);
                    }, $_join);
            ...
            $this->options['join'][] = false !== stripos($join,
                'JOIN') ? $join : $type . ' JOIN ' . $join;
        }
    ...
    public function union($union, $all = false)
    {
        ...
        if (is_string($union)) {
            $prefix = $this->tablePrefix;
            //将__TABLE_NAME__字符串替换成带前缀的表名
            $options = preg_replace_callback("/__([A-Z0-9_-]+)__/sU",
                function ($match) use ($prefix) {
                    return $prefix . strtolower($match[1]);
                }, $union);
