...
class BaiduBCS
{
    ...
    private function authenticate($opt)
    {
        ...
        if (isset ($opt [self::OBJECT]) && !self::validate_object($opt [self::OBJECT])) {
            throw new BCS_Exception ("Invalid object param[" . $opt [self::OBJECT] . "], please check.",
                -1);
        }
        //construct url
        ...
        if (isset ($opt ['fileUpload'])) {
            ...
            if (isset ($opt ["length"])) {
                if ($opt ["length"] > $file_size) {
                    throw new BCS_Exception ("Input opt[length] invalid! It can not bigger than file-size",
                        -1);
                }
                $length = $opt ['length'];
            }
        ...
        return new BCS_ResponseCore ($request->get_response_header(), $request->get_response_body(),
    ...
    public function list_bucket($opt = array())
    {
        ...
        $this->log($response->isOK() ? "List bucket success!" : "List bucket failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function create_bucket($bucket, $acl = null, $opt = array())
    {
        ...
        $response = $this->authenticate($opt);
        $this->log($response->isOK() ? "Create bucket success!" : "Create bucket failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function delete_bucket($bucket, $opt = array())
    {
        ...
        $opt [self::OBJECT] = '/';
        $response           = $this->authenticate($opt);
        $this->log($response->isOK() ? "Delete bucket success!" : "Delete bucket failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function set_bucket_acl($bucket, $acl, $opt = array())
    {
        ...
        $response                 = $this->authenticate($opt);
        $this->log($response->isOK() ? "Set bucket acl success!" : "Set bucket acl failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function get_bucket_acl($bucket, $opt = array())
    {
        ...
        $response                 = $this->authenticate($opt);
        $this->log($response->isOK() ? "Get bucket acl success!" : "Get bucket acl failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function list_object($bucket, $opt = array())
    {
        ...
        $response = $this->authenticate($opt);
        $this->log($response->isOK() ? "List object success!" : "Lit object failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function create_object($bucket, $object, $file, $opt = array())
    {
        ...
        if (isset ($opt ['filename'])) {
            self::set_header_into_opt("Content-Disposition",
                'attachment; filename=' . $opt ['filename'], $opt);
        }
        $response = $this->authenticate($opt);
        $this->log($response->isOK() ? "Create object[$object] file[$file] success!" : "Create object[$object] file[$file] failed! Response: [" . $response->body . "] Logid[" . $response->header ["x-bs-request-id"] . "]",
            $opt);
    ...
    public function create_object_by_content($bucket, $object, $content, $opt = array())
    {
        ...
        $response = $this->authenticate($opt);
        $this->log($response->isOK() ? "Create object[$object] success!" : "Create object[$object] failed! Response: [" . $response->body . "] Logid[" . $response->header ["x-bs-request-id"] . "]",
            $opt);
    ...
    public function create_object_superfile($bucket, $object, $file, $opt = array())
    {
        if (isset ($opt ['length']) || isset ($opt ['seekTo'])) {
            throw new BCS_Exception ("Temporary unsupport opt of length and seekTo of superfile.",
                -1);
        }
        //$opt array
        $this->assertParameterArray($opt);
        ...
        $this->log("File[" . $opt ['fileUpload'] . "], size=[$fileSize], sub_object_size=[$sub_object_size], sub_object_num=[$sliceNum]",
            $opt);
        $object_list = array(
            'object_list' => array()
        );
        for ($i = 0; $i < $sliceNum; $i++) {
            ...
            $this->log("Begin to upload Sub-object[" . $opt [self::OBJECT] . "][$i/$sliceNum][seekto:" . $opt ['seekTo'] . "][Length:" . $opt ['length'] . "]",
                $opt);
            $response = $this->create_object($bucket, $opt [self::OBJECT], $file, $opt);
            if ($response->isOK()) {
                $this->log("Sub-object upload[" . $opt [self::OBJECT] . "][$i/$sliceNum][seekto:" . $opt ['seekTo'] . "][Length:" . $opt ['length'] . "]success! ",
                    $opt);
                $object_list ['object_list'] ['part_' . $i] ['etag'] = $response->header ['Content-MD5'];
                continue;
            } else {
                $this->log("Sub-object upload[" . $opt [self::OBJECT] . "][$i/$sliceNum] failed! ",
                    $opt);
        ...
        $response = $this->authenticate($opt);
        $this->log($response->isOK() ? "Create object-superfile success!" : "Create object-superfile failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function upload_directory($bucket, $dir, $opt = array())
    {
        ...
        foreach ($file_tree as $file) {
            $object                      = $has_sub_directory == true ? substr($file,
                strlen($dir)) : "/" . basename($file);
        ...
        if (isset ($opt ["seek_object_id"]) && isset ($opt ["seek_object"])) {
            throw new BCS_Exception ("Can not set see_object_id and seek_object at the same time!",
                -1);
        }
            ...
            if (isset ($objects [$opt ["seek_object"]])) {
                foreach ($objects as $object => $file) {
                    if ($object != $opt ["seek_object"]) {
                        //当非断点文件，该object已完成上传
                        $this->log("Seeking[" . $opt ["seek_object"] . "]. Skip id[$num/$objectCount]object[$object]file[$file].",
                            $opt);
                        ...
                        //当非断点文件，该object已完成上传
                        $this->log("Found seek id[$num/$objectCount]object[$object]file[$file], begin from here.",
                            $opt);
                        break;
                    }
                    $num++;
            ...
            if (is_int($opt ["seek_object_id"]) && $opt ["seek_object_id"] <= $objectCount) {
                foreach ($objects as $object => $file) {
                    if ($num < $opt ["seek_object_id"]) {
                        $this->log("Seeking object of [" . $opt ["seek_object_id"] . "/$objectCount]. Skip  id[$num/$objectCount]object[$object]file[$file].",
                            $opt);
                        //$result ['skipped'] [] = "[$num/$objectCount]  " . $file;
                        $result ['skipped']++;
                        unset ($objects [$object]);
        ...
        foreach ($objects as $object => $file) {
            ...
            if (isset ($opt [self::IMPORT_BCS_PRE_FILTER]) && function_exists($opt [self::IMPORT_BCS_PRE_FILTER])) {
                ...
                if ($bolRes !== true) {
                    $this->log("User pre_filter_function return un-true. Skip id[$num/$objectCount]object[$object]file[$file].",
                        $opt);
    ...
    public function copy_object($source, $dest, $opt = array())
    {
        ...
        $opt [self::METHOD] = 'PUT';
        self::set_header_into_opt('x-bs-copy-source',
            'bs://' . $source [self::BUCKET] . $source [self::OBJECT], $opt);
        if (isset ($opt ['source_tag'])) {
            self::set_header_into_opt('x-bs-copy-source-tag', $opt ['source_tag'], $opt);
        }
        $response = $this->authenticate($opt);
        $this->log($response->isOK() ? "Copy object success!" : "Copy object failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function set_object_meta($bucket, $object, $meta, $opt = array())
    {
        ...
        $this->log($response->isOK() ? "Set object meta success!" : "Set object meta failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function get_object_acl($bucket, $object, $opt = array())
    {
        ...
        $response                 = $this->authenticate($opt);
        $this->log($response->isOK() ? "Get object acl success!" : "Get object acl failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function set_object_acl($bucket, $object, $acl, $opt = array())
    {
        ...
        $response                 = $this->authenticate($opt);
        $this->log($response->isOK() ? "Set object acl success!" : "Set object acl failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function delete_object($bucket, $object, $opt = array())
    {
        ...
        $opt [self::OBJECT] = $object;
        $response           = $this->authenticate($opt);
        $this->log($response->isOK() ? "Delete object success!" : "Delete object failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function get_object_info($bucket, $object, $opt = array())
    {
        ...
        $opt [self::OBJECT] = $object;
        $response           = $this->authenticate($opt);
        $this->log($response->isOK() ? "Get object info success!" : "Get object info failed! Response: [" . $response->body . "]",
            $opt);
    ...
    public function get_object($bucket, $object, $opt = array())
    {
        ...
        if (isset ($opt ["fileWriteTo"]) && file_exists($opt ["fileWriteTo"])) {
            ...
            $this->log("[$original_file_write_to] already exist, rename it to [" . $opt ["fileWriteTo"] . "]",
                $opt);
        }
        $opt [self::BUCKET] = $bucket;
        $opt [self::METHOD] = 'GET';
        $opt [self::OBJECT] = $object;
        $response           = $this->authenticate($opt);
        $this->log($response->isOK() ? "Get object success!" : "Get object failed! Response: [" . $response->body . "]",
            $opt);
    ...
    private function check_user_acl($acl)
    {
        ...
        foreach ($acl ['statements'] as $key => $statement) {
            ...
            if (!is_array($statement ['action']) || !count(array_diff($statement ['action'],
                    self::$ACL_ACTIONS)) == 0
            ) {
