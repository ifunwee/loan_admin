...
class BCS_RequestCore
{
    ...
    public function streaming_read_callback($curl_handle, $file_handle, $length)
    {
        ...
        $read                   = fread($this->read_stream,
            min($this->read_stream_size - $this->read_stream_read,
                $length)); // Remaining upload data or cURL's requested chunk size
        $this->read_stream_read += strlen($read);
        $out                    = $read === false ? '' : $read;
        // Execute callback function
        if ($this->registered_streaming_read_callback) {
            call_user_func($this->registered_streaming_read_callback, $curl_handle, $file_handle,
                $out);
        }
    ...
    public function streaming_write_callback($curl_handle, $data)
    {
        ...
        if ($this->registered_streaming_write_callback) {
            call_user_func($this->registered_streaming_write_callback, $curl_handle,
                $written_total);
        }
    ...
    public function prep_request()
    {
        ...
        if ($this->proxy) {
            ...
            if (isset ($this->proxy ['user']) && isset ($this->proxy ['pass'])) {
                curl_setopt($curl_handle, CURLOPT_PROXYUSERPWD,
                    $this->proxy ['user'] . ':' . $this->proxy ['pass']);
            }
        }
    ...
    public function process_response($curl_handle = null, $response = null)
    {
        ...
        if (is_resource($this->curl_handle)) {
            ...
            if ($curl_handle && $response) {
                ...
                return new  $class ($this->response_headers, $this->response_body,
                    $this->response_code, $this->curl_handle);
            }
        }
    ...
    public function send_multi_request($handles, $opt = null)
    {
        ...
        do {
            ...
            while (($status = curl_multi_exec($multi_handle,
            ...
            foreach ($to_process as $pkey => $done) {
                $response            = $http->process_response($done ['handle'],
