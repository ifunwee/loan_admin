...
function T($template = '', $layer = '')
{
    ...
    if ('' == $file) {
        ...
        $file = substr_count($file, '/') > 1 ? substr_replace($file, $depr, strrpos($file, '/'),
            1) : str_replace('/', $depr, $file);
    }
            ...
            if (is_array($filters)) {
                foreach ($filters as $filter) {
                    if (function_exists($filter)) {
                        $data = is_array($data) ? array_map_recursive($filter,
...
function array_map_recursive($filter, $data)
{
    ...
    foreach ($data as $key => $val) {
        $result[$key] = is_array($val) ? array_map_recursive($filter,
...
function M($name = '', $tablePrefix = '', $connection = '')
{
    ...
    $guid = (is_array($connection) ? implode('',
            $connection) : $connection) . $tablePrefix . $name . '_' . $class;
...
function U($url = '', $vars = '', $suffix = true, $domain = false)
{
    ...
    if (C('URL_MODEL') == 0) { // 普通模式URL转换
        ...
        if (isset($route)) {
            ...
            $url    = __APP__ . '/' . ($module ? $module . MODULE_PATHINFO_DEPR : '') . implode($depr,
                    array_reverse($var));
        }
        if ($urlCase) {
            $url = strtolower($url);
...
function session($name = '', $value = '')
{
    ...
    if (is_array($name)) { // session初始化 在session_start 之前调用
        ...
        if (C('SESSION_TYPE')) { // 读取session驱动
            $type   = C('SESSION_TYPE');
            $class  = strpos($type,
                '\\') ? $type : 'Think\\Session\\Driver\\' . ucwords(strtolower($type));
            $hander = new $class();
            session_set_save_handler(array(&$hander, "open"), array(&$hander, "close"),
                array(&$hander, "read"), array(&$hander, "write"), array(&$hander, "destroy"),
                array(&$hander, "gc"));
        }
        // 启动session
